...
function sict_policy_notify_page() {
    ...
    <form method="post" action="">
        ...
        <!-- NEW: Statutory Requirement Checkbox -->
        <label for="is_statutory">
    ...
    if (isset($_POST['submit'])) {
        ...
        $is_statutory = isset($_POST['is_statutory']) ? 1 : 0; // NEW: Capture statutory status
        ...
        $notifications[] = array(
            ...
            'is_statutory' => $is_statutory, // NEW: Store statutory status
        );
    ...
    $notifications = get_option('sict_policy_notifications');
    if ($notifications) {
        // NEW: Separate notifications into statutory and non-statutory
        $statutory_notifications = array();
        $non_statutory_notifications = array();
        
        foreach ($notifications as $index => $notification) {
            if (isset($notification['is_statutory']) && $notification['is_statutory']) {
                $statutory_notifications[$index] = $notification;
            } else {
                $non_statutory_notifications[$index] = $notification;
            }
        }
        
        // Display Statutory Notifications Table
        if (!empty($statutory_notifications)) {
            ?>
            <h2>Statutory Policy Notifications</h2>
            <table class="policy-table statutory">
                <tr>
                    <th>Delegate</th>
                    <th>Policy Title</th>
                    <th>Document URL</th>
                ...
                <?php
                foreach ($statutory_notifications as $index => $notification) {
                    ?>
                    <tr>
                        <td><?php echo get_userdata($notification['user_id'])->display_name; ?></td>
                        <td><?php echo $notification['policy_title']; ?> <strong>(Statutory)</strong></td>
        ...
        // Display Non-Statutory Notifications Table
        if (!empty($non_statutory_notifications)) {
            ?>
            <h2>Non-Statutory Policy Notifications</h2>
            <table class="policy-table non-statutory">
                <tr>
                    <th>Delegate</th>
                    <th>Policy Title</th>
                    <th>Document URL</th>
                ...
                <?php
                foreach ($non_statutory_notifications as $index => $notification) {
                    ?>
                    <tr>
                        <td><?php echo get_userdata($notification['user_id'])->display_name; ?></td>
    ...
    if (isset($_GET['edit_index'])) {
        ...
        if (isset($notifications[$key])) {
            ...
            <form method="post" action="">
                ...
                <!-- NEW: Statutory Requirement Checkbox in Edit Form -->
                <label for="is_statutory">
                    <input type="checkbox" name="is_statutory" id="is_statutory" value="1" <?php echo (isset($notification['is_statutory']) && $notification['is_statutory']) ? 'checked' : ''; ?>>
                    This is a statutory requirement
                </label>
                <br><br>
    ...
    if (isset($_POST['update'])) {
        ...
        $notifications[$key]['page_url'] = $page_url;
        $notifications[$key]['renewal_date'] = $renewal_date;
        $notifications[$key]['is_statutory'] = $is_statutory; // NEW: Update statutory status
        update_option('sict_policy_notifications', $notifications);
        ...
        foreach ($notifications as $key => $notification) {
            ...
            if ($renewal_date <= $today) {
                ...
                if ($user) {
                    ...
                    // NEW: Add statutory status to email
                    if (isset($notification['is_statutory']) && $notification['is_statutory']) {
                        $message .= "*** This is a STATUTORY requirement ***\n";
                    }
                    ...
                    if ($sent) {
                        ...
                        $notifications[$key]['sent'] = true;
                        update_option('sict_policy_notifications', $notifications);
                        error_log("Email sent successfully to {$to} for policy: {$notification['policy_title']}");
                    } else {
                        error_log("Error sending email to {$to} for policy: {$notification['policy_title']}");
                    }
                }
            }
        }
